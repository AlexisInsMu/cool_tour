...
class POISelectionAdapter(
    private val onToggle: (POI, Boolean) -> Unit
) : ListAdapter<POI, POISelectionAdapter.ViewHolder>(DiffCallback()) {
    ...
    inner class ViewHolder(private val binding: ItemPoiSelectionBinding) :
        ...
        fun bind(poi: POI) {
            ...
            binding.cbSeleccionar.setOnCheckedChangeListener { _, checked -> onToggle(poi, checked) }
        }
    }

    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): ViewHolder {
        val binding = ItemPoiSelectionBinding.inflate(LayoutInflater.from(parent.context), parent, false)
